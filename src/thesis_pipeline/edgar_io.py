...
def load_ticker_map() -> pd.DataFrame:
    ...
    for obj in data.values():
        ...
        cik = int(obj["cik_str"])
        title = obj["title"]
        recs.append({"ticker": ticker, "cik": cik, "title": title})
        alias = re.sub(r"[^A-Z0-9]", "", ticker)
        if alias and alias != ticker:
            alias_recs.append({"ticker": alias, "cik": cik, "title": title})
    out = pd.DataFrame(recs + alias_recs)
    out = out.sort_values(["ticker", "cik"]).drop_duplicates(subset=["ticker"], keep="first").reset_index(drop=True)
    return out

def get_company_filings(cik: int, include_history: bool = True) -> pd.DataFrame:
    """
    Retrieve SEC submissions for a company by CIK.

    By default this includes both the "recent" array and any historical
    `CIK...-submissions-xxx.json` files listed by the SEC. Older 10-K coverage
    is otherwise understated because the "recent" array does not span the full
    ...
    if include_history:
        for meta in j.get("filings", {}).get("files", []) or []:
            name = meta.get("name")
            if not name:
                continue
            try:
                hist_txt = http_get(f"https://data.sec.gov/submissions/{name}")
                hist = json.loads(hist_txt)
                if hist:
                    frames.append(pd.DataFrame(hist))
            except Exception:
                # Keep the pipeline moving even if one archival chunk fails.
                continue
    ...
    if "accessionNumber" in df.columns:
        df = df.drop_duplicates(subset=["accessionNumber"], keep="first")
    if "filingDate" in df.columns:
        filing_dates = pd.to_datetime(df["filingDate"], errors="coerce")
        df = df.assign(_filing_date=filing_dates).sort_values("_filing_date", ascending=False).drop(columns=["_filing_date"])
    df["cik"] = cik
    return df
...
def get_recent_filings(cik: int) -> pd.DataFrame:
    """
    Backward-compatible alias for the full-company submissions loader.
    """
    return get_company_filings(cik=cik, include_history=True)
...
def compute_parametric_score(
    ...
    """
    Compute a single parametric score used for sweep runs from raw HTML.
    """
    text = html_to_text(html)
    return compute_parametric_score_from_text(
        text=text,
...
def compute_parametric_score_from_text(
    text: str,
    substantive_pats: List[re.Pattern],
    boilerplate_pats: List[re.Pattern],
    section_weight: float,
    ...
    """
    Compute a single parametric score used for sweep runs from normalized text.
    score_mode:
...
def cache_filings_for_years(
    ...
    done_statuses = {"cached", "already_cached"}
    if not recheck_no_filing:
        done_statuses.add("no_filing_for_year")
    done_pairs = set()
    existing_manifest_df = pd.DataFrame()

    if Path(manifest_file).exists():
        try:
            existing_manifest_df = pd.read_csv(manifest_file)
            existing_manifest_df["ticker"] = existing_manifest_df["ticker"].astype(str).str.strip().str.upper()
            existing_manifest_df["year"] = pd.to_numeric(existing_manifest_df["year"], errors="coerce")
            for _, rr in existing_manifest_df.iterrows():
                status = str(rr.get("status", ""))
                ticker = rr.get("ticker")
                year = rr.get("year")
                if status in done_statuses and pd.notna(ticker) and pd.notna(year):
                    done_pairs.add((str(ticker).upper(), int(year)))
            print(f"[CACHE] Loaded existing manifest: {manifest_file} | completed ticker-year pairs: {len(done_pairs)}")
        except Exception as e:
            print(f"[WARN] Could not read existing manifest ({manifest_file}): {e}")
    ...
    for _, r in tqdm(dfm.iterrows(), total=len(dfm), desc="Caching tickers", unit="ticker"):
        tkr, cik = r["ticker"], r["cik"]
        pending_years = [y for y in years if (tkr, int(y)) not in done_pairs]
        if len(pending_years) == 0:
            tqdm.write(f"[SKIP] {tkr}: all requested years already completed")
            continue
        ...
        try:
            filings = get_company_filings(int(cik), include_history=include_history)
        except Exception as e:
            for y in pending_years:
                records.append({
                    "ticker": tkr,
    ...
    if len(existing_manifest_df) > 0:
        combined = pd.concat([existing_manifest_df, manifest_df], ignore_index=True)
    else:
        combined = manifest_df.copy()

    # Keep the latest row per (ticker, year, accession, doc) where possible.
    dedup_cols = [c for c in ["ticker", "year", "accession", "doc"] if c in combined.columns]
    if dedup_cols:
        combined = combined.drop_duplicates(subset=dedup_cols, keep="last")
    combined.to_csv(manifest_file, index=False)
    print(f"[CACHE] Manifest written: {manifest_file} (+{len(manifest_df)} new rows, total {len(combined)})")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--market-cap-file", type=str, default="data/raw/metadata/ticker_addtl_data.csv",
    ...
    ap.add_argument("--substantive-file", type=str, default="assets/terms/substantive_terms.txt",
                    help="Path to a text file of substantive terms (one per line)")
    ap.add_argument("--boilerplate-file", type=str, default="assets/terms/boilerplate_terms.txt",
                    help="Path to a text file of boilerplate terms (one per line)")
    args = ap.parse_args()
